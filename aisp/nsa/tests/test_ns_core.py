...
@pytest.mark.parametrize(
    ...
    [
        (
            np.array([[1, 1, 0], [0, 0, 1]]).astype(bool),
            np.array([1, 0, 0]).astype(bool),
            0.33,
            True,
        ),
        (
        ...
        (
            np.array([[1, 0, 1], [0, 1, 0]]).astype(bool),
            np.array([0, 1, 1]).astype(bool),
            0.9,
            False,
        ),
        (
            np.array([[1, 1, 0], [0, 0, 1]]).astype(bool),
            np.array([1, 0]).astype(bool),
            0.33,
            False,
        ),
    ],
...
def test_check_detector_bnsa_validity(
    x_class: np.ndarray, vector_x: np.ndarray, aff_thresh: float, expected_result: bool
):
    """
...
@pytest.mark.parametrize(
    ...
    [
        (
            np.array([1, 0, 0]).astype(bool),
            np.array([[[1, 1, 1], [1, 1, 1]], [[0, 0, 1], [0, 1, 0]]]).astype(bool),
            0.25,
            0,
        ),
        (
            np.array([0, 1, 1]).astype(bool),
            np.array([[[0, 0, 1], [1, 0, 1]], [[0, 1, 0], [0, 1, 1]]]).astype(bool),
            0.6,
            -1,
        ),
    ],
